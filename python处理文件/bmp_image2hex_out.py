...
def save_hex_data_to_file(hex_data, file_name):
    global img_counter  # 使用全局变量
    with open(file_name, 'a') as file:
        file.write(f'assign cxk[{img_counter}][32399:0] = 32400\'h{hex_data};\n')
    img_counter += 1
    
# 处理多张图片
for i in range(1100,1110):  # 假设你要处理1到9张图片
    ...
    save_hex_data_to_file(hex_data, "cxk.txt") #保存至当前目录下的cxk.txt
